...
public class BasketAnalysis {
	...
	public static void main(String[] args) throws Exception {
		...
		DataSet<Tuple3<String, String, String>> csvInput = env
				.readCsvFile(dataPath)
				/*
				 * .fieldDelimiter() splits your transaction into items, in the
				 * example below we split at "|" which results in PLANT RPA_WID
				 * RPA_BDD RPA_TIX VERSION RPA_RFL
				 */
				.fieldDelimiter("|")
				/*
				 * .includeFields() selects the required indices for the mining
				 * process with 1, indices with 0 are ignored
				 */
				.includeFields(
						"100000000000010000000000000000000001000000000000000000000000000000000000000")
				/*
				 * needed if the first row of your data is used for keys
				 */
				.ignoreFirstLine()
				/*
				 * in .types() your are able to define which class your input
				 * data is transformed to put words to String and number to
				 * Integer
				 */
	...
	public static final class AssignUniqueId
		...
		public Tuple2<Integer, String> map(Tuple3<String, String, String> value) {
			return new Tuple2<>(numItems++, value.f2);
		}
	}
	...
	public static class GroupItemsInListPerBill
		...
		public void reduce(
			...
			for (Tuple3<String, String, String> trans : transactions) {
				if (itemSetMap.containsKey(trans.f2))
					list_items.add(itemSetMap.get(trans.f2));
